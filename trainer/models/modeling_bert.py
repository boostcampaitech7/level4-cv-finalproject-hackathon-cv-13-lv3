...
class BertSelfAttention(nn.Module):
    def __init__(self, config, is_cross_attention=False, position_embedding_type=None):
        super().__init__()
...
class BertSdpaSelfAttention(BertSelfAttention):
    def __init__(self, config, is_cross_attention=False, position_embedding_type=None):
        super().__init__(config, is_cross_attention, position_embedding_type=position_embedding_type)
    ...
    def forward(
        ...
        if is_cross_attention and past_key_value and past_key_value[0].shape[2] == current_states.shape[1]:
            ...
            if past_key_value and not is_cross_attention:
                key_layer = torch.cat([past_key_value[0], key_layer], dim=2)
                value_layer = torch.cat([past_key_value[1], value_layer], dim=2)
...
class BertLayer(nn.Module):
    def __init__(self, config):
        ...
        if self.add_cross_attention:
            ...
            self.crossattention = BertAttention(config, is_cross_attention=True, position_embedding_type="absolute")
