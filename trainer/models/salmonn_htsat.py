...
from .hts_at.htsat_model import HTSATModel
...
class SALMONN(nn.Module):
    ...
    def __init__(
        ...
        # HTS-AT Model Initialization
        assert hts_at_config is not None, "HTS-AT configuration must be provided."
        logging.info('Loading HTS-AT Model')
        self.hts_at = HTSATModel(hts_at_config)
        if self.freeze_hts_at:
            for param in self.hts_at.parameters():
                param.requires_grad = False

        # Layer Normalization for HTS-AT Output
        self.ln_audio = nn.LayerNorm(hts_at_config["hidden_size"])
