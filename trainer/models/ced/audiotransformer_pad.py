...
class AudioTransformer(PreTrainedModel):
    ...
    def forward_padding_mask(self, x: torch.Tensor, padding_mask: torch.Tensor) -> torch.Tensor:
        """
        x.shape: (batch, channels, time_steps)
        padding_mask.shape: (batch, original_time_steps)
        -> 이 함수는 padding_mask의 크기를 x.shape과 맞춰주는 역할을 함.
        """
        batch_size, _, time_steps = x.shape  # x에서 time_steps 가져오기
        original_time_steps = padding_mask.shape[1]

        # 디버그 메시지 (변환 전)
        print(f"[DEBUG] Before forward_padding_mask - x.shape: {x.shape}, padding_mask.shape: {padding_mask.shape}")

        # 크기가 다르면 조정
        if original_time_steps != time_steps:
            print(f"[WARNING] Adjusting padding_mask from {original_time_steps} to {time_steps}")
            ratio = original_time_steps / time_steps
            indices = torch.arange(time_steps, device=x.device) * ratio
            indices = indices.long().clamp(max=original_time_steps - 1)
            padding_mask = padding_mask[:, indices]

        # BEATs 방식으로 view() 적용 후 마지막 차원 기준으로 all() 연산
        padding_mask = padding_mask.view(batch_size, time_steps, -1)
        padding_mask = padding_mask.all(-1)

        # 디버그 메시지 (변환 후)
        print(f"[DEBUG] After forward_padding_mask - x.shape: {x.shape}, padding_mask.shape: {padding_mask.shape}, padding sum: {padding_mask.sum()}")

        return padding_mask


    def forward_features(self, x: torch.Tensor, padding_mask=None) -> torch.Tensor:
        ...
        if padding_mask is not None:
            if padding_mask.shape[1] != x.shape[1]:  # Transformer 입력 전 크기 재조정
                print(f"[WARNING] Padding mask shape mismatch: {padding_mask.shape} vs {x.shape}")
                padding_mask = self.forward_padding_mask(x, padding_mask)
            print(f"[DEBUG] Before blocks - x.shape: {x.shape}, padding_mask.shape: {padding_mask.shape}")
        ...
        # Sequential이 아니면 padding_mask를 전달
        if isinstance(self.blocks, nn.Sequential):
            x = self.blocks(x)  # padding_mask 전달 X
        else:
            x = self.blocks(x, padding_mask=padding_mask)  # padding_mask 전달

        if padding_mask is not None:
            print(f"[DEBUG] After blocks - x.shape: {x.shape}, padding_mask.shape: {padding_mask.shape}")
    ...
    def forward_spectrogram(self, x: torch.Tensor, padding_mask=None) -> torch.Tensor:
        ...
        if x.shape[-1] > self.maximal_allowed_length:
            ...
            x = self.forward_features(x, padding_mask=padding_mask)
    ...
    def forward(self, x, padding_mask=None):
        ...
        if padding_mask is not None and not hasattr(self, "_mask_fixed"):
            print(f"[DEBUG] Before forward_padding_mask - x.shape: {x.shape}, padding_mask.shape: {padding_mask.shape}")
            padding_mask = self.forward_padding_mask(x, padding_mask)  # 패딩 변환
            self._mask_fixed = True  # 중복 변환 방지
            print(f"[DEBUG] After forward_padding_mask - x.shape: {x.shape}, padding_mask.shape: {padding_mask.shape}")

        if self.training:
            x = self.spectransforms(x)

        x = self.forward_spectrogram(x, padding_mask=padding_mask)
        return x
