    ...
    def __init__(
        ...
        if not only_preprocessor: # 전처리 모드가 아닌 경우 (아마 Audio Encoder가 Preprocessor인 듯)
            ...
            if self.low_resource:
                ...
                self.llama_model = load.from_pretrained(
                    llama_path,
                    torch_dtype=torch.float16, # FP16 precision
        ...
        with self.maybe_autocast():
            if self.use_speech_Qformer:
                ...
                if self.window_level_Qformer:
                    ...
                    kernel = (1, kernel)
                    stride = (1, stride)
                    speech_embeds_tr = speech_embeds.transpose(1, 2).unsqueeze(2) # 차원 변경: [B, T, C] -> [B, C, 1, T]
                    speech_embeds_overlap = F.unfold(speech_embeds_tr, kernel_size=kernel, dilation=1, padding=0, stride=stride) # unfold로 윈도우 분할
                    _, _, L = speech_embeds_overlap.shape
                    speech_embeds_overlap = speech_embeds_overlap.view(B, -1, kernel[1], L) # 윈도우 별로 재구성: [B, C*kernel, L] -> [B, C, kernel[1], L]
                    speech_embeds_overlap = torch.permute(speech_embeds_overlap, [0, 3, 2, 1]) # 차원 순서 변경: [B, L, kernel[1], C]
                    speech_embeds = speech_embeds_overlap.reshape(-1, kernel[1], C) # 최종 형태로 변형: [B*L, kernel[1], C]
                    speech_atts = torch.ones(speech_embeds.size()[:-1], dtype=torch.long, device=speech_embeds.device) # 각 윈도우에 대한 어텐션 마스크 생성
                    # speech_embeds 형태가 [B*L, kernel[1], C]일 때
                    # [:-1]로 마지막 차원(C)을 제외 → [B*L, kernel[1]]
