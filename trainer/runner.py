...
class Runner:
    def __init__(self, cfg, model, datasets, job_id, dryrun):
        ...
        # LLM이 대화 형식의 입력을 처리하도록 설계되어 있기 때문에 Prompt를 챗봇 형식으로 변환
        if test_prompt_path:
            try:
        ...
        if self.use_distributed:
            # 모델을 분산 학습 환경에 맞게 설정
            self.model = DDP( # DPP로 Wrapping
                self._model, device_ids=[self.config.config.run.gpu] # 현재 GPU ID
            )
        else:
    ...
    def valid_epoch(self, epoch, split, decode=False, save_json=False):
        if not self.dryrun:
            # 평가를 위해 분산 학습 시 DDP로 Wrapping된 모델을 언패킹 -> 원본 모델 사용
            model = self.unwrap_dist_model(self.model) # 아래에서 model.prompt_dict, generate 사용하기 위함
            model.eval() # 평가 모드
        ...
        for samples in metric_logger.log_every(dataloader, self.config.config.run.log_freq, header=header):
            samples = prepare_sample(samples, cuda_enabled=self.cuda_enabled) # 데이터 전처리

            if not self.dryrun:
                with torch.cuda.amp.autocast(enabled=self.use_amp):
                    forward_result = model(samples, verbose=True) # 예측
                loss = forward_result.get("loss", 0) # 손실 함수
                correct = forward_result.get("correct", 0) # 정답 개수
                total = forward_result.get("total", 1) # 총 데이터 개수
                res = {
                    "id": samples["id"], # 데이터 ID
            ...
            if decode: # Test 모드 (디코딩, inference)
                if model.prompt_dict:
                    if self.test_prompt_dict is None:
                        ...
                        if "Q" in samples:
                            # Q가 있는 경우, prompt에 Q를 삽입
                            prompts = [p.format(q) if "{}" in p else p for p, q in zip(prompts, samples["Q"])]
                else:
                    prompts = None

                # 텍스트 생성
                text = model.generate(samples, self.config.config.run, prompts=prompts) # 예측
                res["text"] = text
                res["prompt"] = prompts
        ...
        # 분산 학습 환경에서 결과를 동기화
        if is_dist_avail_and_initialized():
            # 모든 GPU에서 결과를 합산해 평균을 계산한 다음 각 GPU에 전달
