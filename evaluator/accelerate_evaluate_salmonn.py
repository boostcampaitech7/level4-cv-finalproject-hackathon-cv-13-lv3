...
def main():
    ...
    if tokenizer.bos_token_id is None:
            ...
            # LLM의 임베딩 크기 재조정
            llama_model.module.base_model.model.resize_token_embeddings(len(tokenizer))
    ...
    with torch.no_grad():
        ...
        for samples in tqdm(dataloader, disable=not accelerator.is_local_main_process):
            ...
            llama_model.module.config.eos_token_id = llama_model.module.config.eos_token_id if isinstance(llama_model.module.config.eos_token_id, int) else llama_model.module.config.eos_token_id[0]
            with accelerator.autocast():
                outputs = llama_model.module.generate(
                    inputs_embeds=embeds,
                    pad_token_id=llama_model.module.config.eos_token_id,
