...
#     try:
#         speech_encoder = torch_tensorrt.load("./trt_models/speech_trt.ep").module().eval().cuda()
#         llm = torch_tensorrt.load("./trt_models/llm_trt.ep").module().eval().cuda()
...
#     # else:
#     #     speech_encoder, llm = load_aot_models()
...
def main():
    ...
    with torch.no_grad():
        ...
        for samples in tqdm(dataloader, disable=not accelerator.is_local_main_process):
            ...
            # Encode speech
            print("spectrogram.shape {}".format(spectrogram.shape))
            print("spectrogram type {} and dtype {}".format(type(spectrogram), spectrogram.dtype))
            with accelerator.autocast():    
                speech_embeds, speech_atts = encode_speech(
                    spectrogram, raw_wav=raw_wav, audio_padding_mask=audio_padding_mask
                )
            ...
            print("embeds.shape {}".format(embeds.shape))
            print("embeds type {} and dtype {}".format(type(embeds), embeds.dtype))
            print("attns.shape {}".format(attns.shape))
            print("attns type {} and dtype {}".format(type(attns), attns.dtype))    
            print("pad_token_id {}".format(llama_model.module.config.eos_token_id[0]))
            print("generate_cfg {}".format(generate_cfg))
            with accelerator.autocast():
                outputs = llama_model.module.generate(
                    inputs_embeds=embeds,
                    pad_token_id=llama_model.module.config.eos_token_id[0],
