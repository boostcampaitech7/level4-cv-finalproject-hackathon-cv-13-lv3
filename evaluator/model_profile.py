...
def main():
    ...
    accelerator = Accelerator(mixed_precision='fp16' if cfg.config.run.get("amp", False) else 'no', kwargs_handlers=[profile_kwargs])
    ...
    with accelerator.profile() as prof:
        with torch.no_grad():
            for batch_idx, samples in enumerate(tqdm(dataloader, disable=not accelerator.is_local_main_process)):
    ...
    prof.export_chrome_trace("profile_results/trace.json")
    with open("profile_results/profile_stats.txt", "w") as f:
